...
//PDUltrasoN
float kpULTRA = 2.0, kdULTRA = 0, erreurancien, erreur, PD = 0, P, D;
...
void vitesseDroitPID(float consigne) {
  int vitesseActuel = (((counterR - previousCounterR) * diametreRoue * PI ) / resolution) / (periode / 1000);
  int erreur = consigne - vitesseActuel;
    /*
    if (((erreur * 1000) / periode) >= accelerationMax) {
      erreur = accelerationMax;
    } else if (((erreur * 1000) / periode) < accelerationMax){
  ...
  /*
    Serial.print(500);
    Serial.print(" ");
    Serial.print(vitesseActuel);
    Serial.print(" ");
  */
}

void vitesseGauchePID(float consigne) {
  int vitesseActuel = (((counterL - previousCounterL) * diametreRoue * PI ) / resolution) / (periode / 1000);
...
void PdUltrason() {
  ...
  vitesseLineaire += kpULTRA * erreurLineaire;

  if (vitesseLineaire >= 80)
    vitesseLineaire = 80;
  if (vitesseLineaire < - 80)
    vitesseLineaire = - 80;
...
void loop()
{
  vitesseDroitPID(500.00);
  vitesseGauchePID(500.00);
/*
  ...
  if ((checkpoint1 == 1) && (checkpoint2 == 0))
    switch (etatcapteur) {
      case 0:
        Avancer(140);
        break;
  ...
  if ((checkpoint2 == 0) && (checkpoint1 == 1)) {
    if ((distF < 150) && (distF != 0)) {
      Stop();
      delay (5000);
      checkpoint2 = 1;
    }
