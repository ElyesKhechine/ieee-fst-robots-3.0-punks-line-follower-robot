...
void PdUltrason() {
  ...
  ferreurancien = erreur ;
  float erreur = distL - 68;
  float P = 0.1 * erreur;
  float D = kdULTRA * (erreur - ferreurancien);
  vitesseAngulaire += P + D;
  //vitesseAngulaire =constrain ( P + D+vitesseAngulaire,-40,40);
  if (vitesseAngulaire > 2)
    vitesseAngulaire = 2;
  if (vitesseAngulaire < - 2)
    vitesseAngulaire = - 2;
  ...
  vitesseLineaire += kpULTRA * erreurLineaire;
   //vitesseLineaire =constrain (kpULTRA * erreurLineaire+vitesseLineaire,-20,20);//+ kdULTRA * (erreurLineaire - erreurLineaireAncient);

  if (vitesseLineaire >50)
    vitesseLineaire = 50;
  if (vitesseLineaire < - 30)
    vitesseLineaire = - 30;
/*
   Serial.print(distF);
    Serial.print(" ");
...
*/
  ...
  vitesseDroite = vitesseLineaire + vitesseAngulaire*8;
  vitesseGauche = vitesseLineaire - vitesseAngulaire*8;
