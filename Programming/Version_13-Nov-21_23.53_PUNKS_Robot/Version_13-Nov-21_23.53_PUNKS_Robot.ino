...
//maxcounterL(8.75*PI)*r
void vmazeforward(float r){
  float wL = (((counterL - previousCounterL) * diametreRoue * PI ) / resolution) / (periode / 1000);
  float vL=r*wL;
  float vR=r*((sqrt(2)/50)*r-vL)*(wL/vL);
  analogWrite(moteurDroitAvant, map(vR,0,70,0,40));
  analogWrite(moteurGaucheAvant, map(vL,0,70,0,40));
  analogWrite(moteurDroitDeriere, 0);
  analogWrite(moteurGaucheDeriere, 0);
 previousCounterL=counterL;
}
void vmazebackward(float r){
  float wL = (((counterL - previousCounterL) * diametreRoue * PI ) / resolution) / (periode / 1000);
  float vL=r*wL;
  float vR=r*((sqrt(2)/50)*r-vL)*(wL/vL);
  analogWrite(moteurDroitAvant, 0);
  analogWrite(moteurGaucheAvant, 0);
  analogWrite(moteurDroitDeriere, map(vR,0,70,0,40));
  analogWrite(moteurGaucheDeriere, map(vL,0,70,0,40));
  previousCounterL=counterL;
}
...
void loop() {
  ...
  distanceleft();
  distancefront();
  float er=(distL-50)-(distR-50);
  /*ISRtrackAR();
  ISRtrackAL();*/
  /*if (er>70){
      vmazebackward(distR+42);
  }
  if (er<-70){
      vmazebackward(distL+42);
  }
  */
    if ((er>=-10)&&(er<=10) && (distF>=70))
        Avancer(40);
    if ((er>=-30)&&(er<=30) && (distF>=70))
        vmazeforward(520+(160-abs(er)));
    if ((er>50)&& (distF<=70))
        vmazebackward(distR+42);
    if ((er<-50)&& (distF<=70))
        vmazebackward(distL+42);
